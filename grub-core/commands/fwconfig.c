		   ...
		   int argc, char **argv)
{
  ...
  if (grub_memcmp(fwsig, signature, sizeof(signature)) != 0)
    return grub_error (GRUB_ERR_BAD_DEVICE,
		       N_("invalid fwconfig signature: got 0x%x%x%x%x"),
		       fwsig[0], fwsig[1], fwsig[2], fwsig[3]);

  /* Find out how many file entries we have */
  grub_outw(DIRECTORY_INDEX, SELECTOR);
  value = grub_inb(DATA) | grub_inb(DATA) << 8 |
    grub_inb(DATA) << 16 | grub_inb(DATA) << 24;
  ...
  for (i=0; i<value; i++)
      ...
      for (j=0; j<sizeof(file); j++)
	{
      ...
      /* Check if it matches what we're looking for, and read the file if so */
      if (grub_strncmp(file.name, argv[0], sizeof(file.name)) == 0)
	{
	  ...
	  if (!data)
	      return grub_error (GRUB_ERR_OUT_OF_MEMORY,
				 N_("can't allocate buffer for data"));
